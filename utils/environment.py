...
# -------------------------------[ Audiobooks ]-------------------------------
AUDIOBOOKS_DIR = os.environ.get("AUDIOBOOKS_DIR", "/media/audiobooks")
...
QBITTORRENT_DOWNLOADS_DIR = os.environ.get(
    "QBITTORRENT_DOWNLOADS_DIR", "/media/downloads"
)
...
# If not provided, Jackett will not be available as a source for searches. This is required to access Jackett's API.
...
# ------------------------------[ FlareSolverr ]-----------------------------
FLARESOLVERR_ADDRESS = os.environ.get("FLARESOLVERR_ADDRESS")
...
# ----------------------------[ ENV VALIDATION ]----------------------------
def validate_int(name, value)-> int:
    try:
        int(value)
    except ValueError:
        logger.critical(
